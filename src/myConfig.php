...
'modelinfo' => [
    ...
    private static function init( ): void {
        ...
        self::setConfigDirFile( __DIR__.'/config' );

        self::save(self::$configs);
        // self::setUserConfigFile();
         $user_path  = myAPP_PATH."/myconfig"; 
        if (! empty(  $user_path)) {
                 is_dir($user_path) && self::setConfigDirFile( $user_path );
        }

    }

   public static function  setConfigDirFile ($path = ''): array {
        if (is_dir($path) == false) {
            die ('Failed to create directory: ' . $path);
        }
        $arr = [];
        $arr[0] = self::getAllConfig();
        // for $path /*   set config ['filename' ] = include $path/filename.php   */
        ...
        foreach ($configFiles as $configFile) {
            ...
            $configArray[$fileName] = include $configFile;
        }
...
private static function setUserConfigFile(  ){
    ...
    public static function getDataConfig(string $type = '',$default  ) {
        if (empty($type)) {
            return [];
        }
        $data = self::getAllConfig ();
